  ...
  <div class="app-container">
    <el-card class="box-card">
      ...
      <el-dialog title="警告" :visible.sync="confirmDelete" width="30%">
        <span>确定要删除 <b>{{ deleteRowData.while_ip }}</b> 吗？</span>
      ...
      >
        ...
        >
          <el-form-item label="IP地址" prop="while_ip">
            <el-input
              v-model.trim="temp.while_ip"
              style="width: 300px"
              placeholder="IP"
            />
          </el-form-item>
...
<script>
import { getWhile, createWhile, updateWhile, deleteWhile } from '@/api/while'
...
export default {
  ...
  methods: {
    getList(obj) {
      ...
      getWhile(this.listQuery).then(response => {
    ...
    handleUpdate(row) {
      ...
      this.temp.while_ip = row.while_ip
    ...
    createData() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          createWhile(this.temp).then(response => {
            this.restTemp()
            this.restData()
            this.getList()
            this.$message({
